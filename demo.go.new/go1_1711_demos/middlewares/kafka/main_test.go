...
func TestCollectSuccessfully(t *testing.T) {
	...
	if resp.Code != 200 {
		t.Errorf("Expected HTTP status 200, found %d", resp.Code)
	}
	body := resp.Body.String()
	t.Log("response:", body)
	if body != "Your data is stored with unique identifier important/0/1" {
...
func TestCollectionFailure(t *testing.T) {
	...
	if resp.Code != 500 {
		t.Errorf("Expected HTTP status 500, found %d", resp.Code)
	}
}
...
func TestWrongPath(t *testing.T) {
	...
	if resp.Code != 404 {
		t.Errorf("Expected HTTP status 404, found %d", resp.Code)
	}
}
