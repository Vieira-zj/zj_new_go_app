...
// Refer: https://github.com/ayanamist/k8s-utils/blob/3d5e35f70386d8dceebdc070cadecb9be0c518bd/pkg/streamlister
...
func StreamList(ctx context.Context, client rest.Interface, resource, namespace string, opts metav1.ListOptions, param ParamInterface) error {
	...
	rc, err := client.Get().
		...
		Stream(ctx)
	if err != nil {
		return fmt.Errorf("client.Stream error: %w", err)
	}
	defer rc.Close()
	...
	unknowStream := unknownStreamUnmarshaler{
	...
	if err := unknowStream.Unmarshal(newStreamBuffer(rc, -1)); err != nil {
