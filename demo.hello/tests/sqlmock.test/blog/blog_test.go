...
func (a *api) assertJSON(actual []byte, data interface{}, t *testing.T) {
	fmt.Println("actual results:", string(actual))
	expect, err := json.Marshal(data)
	if err != nil {
		t.Fatalf("an error '%s' was not expected when marshaling expected json data", err)
	}

	if bytes.Compare(expect, actual) != 0 {
		t.Errorf("the expected json: %s is different from actual %s", expect, actual)
	}
}
...
func TestShouldRespondWithErrorOnFailure(t *testing.T) {
	...
	// before we actually execute our api function, we need to expect required DB actions
	mock.ExpectQuery("^SELECT (.+) FROM posts$").WillReturnError(errors.New("mock error"))
	...
	data := struct {
		Error string
	}{"failed to fetch posts: mock error"}
	app.assertJSON(w.Body.Bytes(), data, t)
