...
import (
	...
	"demo.hello/tests/gomock.test/user/imp1"
	renamed2 "demo.hello/tests/gomock.test/user/imp2"
	"demo.hello/tests/gomock.test/user/imp3"
	impfour "demo.hello/tests/gomock.test/user/imp4"
)

// mock gen cmd:
// mockgen -source=user.go -destination=mock_user/mock_user.go

// Index a bizarre interface to test corner cases in mockgen.
// This would normally be in its own file or package, separate from the user of it (e.g. io.Reader).
...
// Embed an interface with an embedded interface.
...
// Embedded for mock embedded interface.
...
// GrabPointer for use above interface.
