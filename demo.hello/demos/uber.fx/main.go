...
// Refer: https://pkg.go.dev/go.uber.org/fx#section-readme
...
// function for the standard library's logger. Since NewLogger
...
// know how to construct a logger and needs an HTTP handler, it will fail to start.
//
...
// once, and both the handler and the logger would be cached and reused as necessary.
...
//
// Unlike constructors, invocations are called eagerly.
...
func main() {
	app := fx.New(
		...
		// NewMux, we also register Lifecycle hooks to start and stop an HTTP server.
		fx.Invoke(Register),

		// This is optional. With this, you can control where Fx logs its events.
		// In this case, we're using a NopLogger to keep our test silent.
		// Normally, you'll want to use an fxevent.ZapLogger or an fxevent.ConsoleLogger.
		fx.WithLogger(
			func() fxevent.Logger {
	...
	// don't want this example to run forever, we'll use the more-explicit Start and Stop.
	startCtx, cancel := context.WithTimeout(context.Background(), time.Duration(15)*time.Second)
	defer cancel()
	...
	stopCtx, cancel := context.WithTimeout(context.Background(), time.Duration(15)*time.Second)
	defer cancel()
